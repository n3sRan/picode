...
- 当前状态：Phase 0-9 已完成，MVP 已封板。代码、确定性测试、文档和本地 npm tarball 发布流程已整理；真实 API smoke run 和视频录制仍按需人工执行，不作为默认测试或发布包内容。
...
| M3 | Agent Loop 与终止条件 | fake model 可完成串行读、改、命令验证和 `finish`（已完成） |
| M4 | 基本会话持久化与 CLI | 会话可新建、列出、恢复；单任务与交互入口可用（已完成） |
| M5 | 全量验证与隔离 E2E | 本地 tarball 可在仓库外 demo 从 `picode` bin 完成真实任务（已由用户手动验证） |
| M6 | 文档、演示与提交检查 | 行为与文档一致，演示方案、打包说明和提交物就绪（已完成） |
| M7 | 上下文可观测性与压缩 | `finish` 终态显示当前上下文；`/compact` 可安全整理 session；可选自动压缩在阈值前触发且关闭时行为不变（已完成） |
| M8 | 终端详细度与 finish 展示 | `--verbose`、`/verbose`、`/verbose off` 可切换进程级展示；普通工具可折叠；finish 中间输出按模式隐藏；独立 `[context]` 始终显示（已完成） |
| M9 | 启动 session 选择与历史回放 | 默认启动新 session；`--resume[ <id>]` 显式恢复；`/resume <id>` 和 CLI 恢复后按 verbose 规则回放一次历史；无 session 时恢复请求报错（已完成） |
...
状态：已完成（CLI UI 重构、限制与文件搜索边界、基础结构清理、LLM 运行参数配置、取消/超时处理、批次调用关联、macOS 大小写路径保护，以及符号链接和崩溃恢复回归测试已完成）。
...
状态：已完成（批次 H、I、J、K 已完成；SPEC、ARCHITECTURE、README 和本计划已同步）。
...
- 隔离 demo 可以从打包后的 bin 展示读、改、验证、压缩、恢复和 `finish`，且开发仓库无演示产物（已复核）。

## 11. Phase 8：终端详细度与 finish 输出重构

状态：已完成（代码、确定性测试和发布说明已完成；视频录制仍为人工事项）。
...
- `[context]` 独立输出在终态之后可观察（成功、部分和失败 finish 均复用同一 renderer 分支，已验证）。
- build、typecheck、全量确定性测试已通过；仓库外打包 bin 的 verbose 开启/关闭流程已复核。
...
- build、typecheck、全量确定性测试和本地 tarball 发布检查通过；仓库外打包 bin 的新建、恢复和历史展示流程已由用户手动复核。
