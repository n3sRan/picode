...
| M5 | 全量验证与隔离 E2E | 本地 tarball 可在仓库外 demo 从 `picode` bin 完成真实任务 |
...
出口：build、typecheck 和测试通过，不访问真实 API。
...
5. 实现 20 次请求、10 分钟活跃时间、连续错误、无 finish 和重复调用限制。
...
出口：scripted fake provider 能确定性完成“读文件 → 编辑 → 运行命令 → finish”。
...
出口：构建后的 bin 能用 fake provider 在临时项目完成全流程。
...
出口：开发仓库和参考项目中没有演示任务产物；全流程从打包后的 bin 入口完成。
